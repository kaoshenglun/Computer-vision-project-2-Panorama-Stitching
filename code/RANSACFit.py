...
def RANSACFit(p1, p2, match, maxIter=200, seedSetSize=None, maxInlierError=30, goodFitThresh=None):
    ...
    for i in range(maxIter):
        ...
        eta = ComputeAffineMatrix(p1[beta[:,0].astype('int64'),:], p2[beta[:,1].astype('int64'),:]) # arrays used as indices must be of integer (or boolean) type
        ...
        if np.sum(epsilon) + alpha >= goodFitThresh:
            ...
            eta = ComputeAffineMatrix(p1[zeta[:,0].astype('int64'), :], p2[zeta[:,1].astype('int64'), :]) # arrays used as indices must be of integer (or boolean) type
